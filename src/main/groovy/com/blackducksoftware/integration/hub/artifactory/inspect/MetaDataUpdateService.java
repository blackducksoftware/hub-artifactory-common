...
import com.blackducksoftware.integration.hub.artifactory.inspect.metadata.ArtifactMetaDataFromNotifications;
...
public class MetaDataUpdateService {
    ...
    public MetaDataUpdateService(final ArtifactoryPropertyService artifactoryPropertyService, final CacheInspectorService cacheInspectorService, final ArtifactMetaDataService artifactMetaDataService, final MetaDataPopulationService metadataPopulationService) {
        ...
        if (inspectionStatus.isPresent() && inspectionStatus.get().equals(InspectionStatus.SUCCESS)) {
            try {
                ...
                try {
                    ...
                    throw new IntegrationException(String.format("Could not find timestamp property on %s. Black Duck artifactory metadata is likely malformed and requires re-inspection. Run the blackDuckDeleteInspectorProperties rest endpoint to re-inspect all configured repositories or delete the malformed properties manually.", repoKeyPath.toPath()), npe);
                }
